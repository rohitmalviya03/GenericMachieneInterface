...
public class DraegerSerialComm {
    ...
    private static void sendICCresponse(SerialPort serialPort) {
        byte[] idCommand = {(byte) 0x01, (byte) 0x51, (byte) 0x35, (byte) 0x32, (byte) 0x0D};
        serialPort.writeBytes(idCommand, idCommand.length);
        System.out.println("IC Response Command sent");
    }
    private static void sendRequestData(SerialPort serialPort) {
        byte[] idCommand = {(byte) 0x1B, (byte) 0x24, (byte) 0x33, (byte) 0x46, (byte) 0x0D};
        serialPort.writeBytes(idCommand, idCommand.length);
        System.out.println("Request data Command sent");
    }
