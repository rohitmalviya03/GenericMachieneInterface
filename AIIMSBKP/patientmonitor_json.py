...
def parse_hl7_to_json(hl7_message,cr_no,m_id):
    ...
    try:
        ...
        machine_id = m_id  # Example machine ID
        patient_id = cr_no  # Example patient ID
...
def receive_data(client_socket, running,cr_no,m_id):
            ...
            while START_BLOCK in buffer and END_BLOCK + CARRIAGE_RETURN in buffer:
                ...
                if start != -1 and end != -1 and end > start:
                    ...
                    try:
                        ...
                        json_output = parse_hl7_to_json(received_message,cr_no,m_id)
                        if json_output:
...
def main():
   ...
   # host = "192.168.0.103"  # Replace with the actual IP address
    ...
    if len(sys.argv) != 4:
        print("Usage: python script.py <server_ip> <server_port> <cr_no> <mid>")
        sys.exit(1)
    ...
    try:
        ...
        receive_thread = threading.Thread(target=receive_data, args=(client_socket, running,cr_no,m_id))
